  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold">{t("tools.sql.pageTitle")}</h1>
        <p className="text-muted-foreground">
          {t("tools.sql.pageDescription")}
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <span>{t("tools.sql.inputQuery")}</span>
            </CardTitle>
            <CardDescription>{t("tools.sql.inputDescription")}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleFormat} className="flex-1">
                ...
                {t("tools.sql.formatButton")}
              </Button>
              <Button variant="outline" onClick={handleClear}>
                <RotateCcw className="w-4 h-4 mr-2" />
                {t("tools.sql.resetButton")}
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span>{t("tools.sql.formattedQuery")}</span>
              {formattedQuery && (
                <Button variant="outline" size="sm" onClick={handleCopy}>
                  <Copy className="w-4 h-4 mr-2" />
                  {t("tools.sql.copyButton")}
                </Button>
              )}
            </CardTitle>
            <CardDescription>
              {t("tools.sql.outputDescription")}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("tools.sql.exampleTitle")}</CardTitle>
          <CardDescription>{t("tools.sql.exampleDescription")}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">
                {t("tools.sql.beforeFormat")}
              </h4>
              <code className="block p-3 bg-muted rounded text-xs">
                select u.id,u.name,p.title from users u left join posts p on
                u.id=p.user_id where u.active=1 order by u.created_at desc limit
                10;
              </code>
            </div>
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">
                {t("tools.sql.afterFormat")}
              </h4>
